...
###
# Defining the model here with the keras functional api
###
...
####
def classA1_trained_cnn(load_from_file = False, 
                        verbose = True,
                        reg_wdecay_beta = 0, 
                        reg_dropout_rate = 0, 
    ...
    y_hat_train = train_labels[:,2]
    y_hat_test = test_labels[:,2]
    ...
    save_name = os.path.join('saved', 'class_simple_A1ResNet_rwd%.1e_rdp%.1f_rbn%d_daug%d%d' % (reg_wdecay_beta,reg_dropout_rate,int(reg_batch_norm),int(data_aug),early_stop))
    ...
    if verbose:
        ...
        for i, ax in enumerate(axes.flat):
            ...
            label = y_hat_train_sample[i]
            # Show image
            ax.imshow(image, cmap='gray')
            ax.text(0.5, -0.12, f'Label: {class_names[label]}', ha='center',
    ...
    if load_from_file and os.path.isfile(net_save_name):
        ...
        # Define training regime: type of optimizer, loss function to optimize and type of error measure to report during
        # training
    ...
    if verbose:
        ...
        y_test_pred = net.predict(x_test[:9])
        ...
        for i, ax in enumerate(axes.flat):
            ...
            # Show image
            ax.imshow(image, cmap='gray')
            ax.text(0.5, -0.12, f'Label: {class_names[label]}, Pred: {class_names[pred]}', ha='center',
                    transform=ax.transAxes, color='black')
            ax.set_xticks([])
            ax.set_yticks([])
        plt.show()
...
if __name__ == "__main__":
   classA1_trained_cnn(load_from_file=True)
